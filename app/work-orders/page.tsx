...
type WOFilter = "all" | "mine" | "unassigned" | "delayed" | "ready_to_invoice";
...
export default function Home() {
  const { woFilter, setWoFilterAndUrl } = useUrlWoFilter();
    ...
    const filteredWorkOrders = useMemo(() => {
        ...
        if (woFilter === "delayed") {
            // in_progress por más de 3 días (proxy)
    ...
    const refreshWorkOrders = useCallback(
        async (cid?: string, silent = false) => {
            ...
            try {
                ...
                const { data, error } = await fetchWorkOrders(company);
    ...
    const saveAssignment = useCallback(
        async (workOrderId: string) => {
            ...
            try {
                ...
                const { error } = await setWorkOrderAssignee(workOrderId, assigned_to);
    ...
    const updateStatus = useCallback(
        async (id: string, status: WorkOrderStatus) => {
            ...
            const { error } = await setWorkOrderStatus(id, status);
            ...
            >
                ...
                >
                    ...
                    <div style={{ marginLeft: "auto", fontSize: 12, opacity: 0.8 }}>
                        Mostrando: <b>{filteredWorkOrders.length}</b> · Total:{" "}
                        <b>{workOrders.length}</b> · Filter: <b>{woFilter}</b> · myUserId:{" "}
                        <b>{myUserId ?? "null"}</b>
                    </div>
                </div>
